...
public interface TestRepository extends JpaRepository<TestModel, String> {
    ...
    @Query(value = "SELECT * from tests where tests.disabled = 'false' and (visibility = 'PUBLIC' or (select COUNT(*) from allowed_users where allowed_users.test_id = tests.id and CAST(allowed_users.user_id AS varchar(255)) = ?1) = 1)", nativeQuery = true)
